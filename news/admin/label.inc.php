...
if($action=='mod'){
	...
	if(strstr($postdb[tplpart_2code],'$picurl')&&!strstr($postdb[tplpart_1code],'$picurl'))
	{
		if(strstr($postdb[tplpart_2code],'$content')){
			$SQL2=" SELECT A.*,R.content FROM {$_pre}content A LEFT JOIN {$_pre}content_1 R ON A.id=R.id $SQL AND A.ispic=1 ORDER BY $order $asc LIMIT 1 ";
		}else{
			$SQL2=" SELECT A.* FROM {$_pre}content A $SQL AND A.ispic=1 ORDER BY $order $asc LIMIT 1 ";
		}		
	}
	...
	if( strstr($postdb[tplpart_1code],'$content')||strstr($postdb[tplpart_2code],'$content') ){
		$SQL=" SELECT A.*,R.content FROM {$_pre}content A LEFT JOIN {$_pre}content_1 R ON A.id=R.id $SQL $SQLPIC ORDER BY $order $asc LIMIT $rows ";
	}else{
		$SQL=" SELECT A.* FROM {$_pre}content A $SQL $SQLPIC ORDER BY $order $asc LIMIT $rows ";
	}
